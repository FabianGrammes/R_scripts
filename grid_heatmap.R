...
ns.counts <- function(mat, col =c('dodgerblue2','white','red'), scale = TRUE ){   
    ...
    if( scale ){
        ...
        mat <-  mat[or,]
        v <- max(abs( c(max(as.vector(mat)), min(as.vector(mat)))) )
        # log scale breaks
        breaks = seq(-v, v, length.out = 99)
        col.X <- my.col(99)[cut(mat, breaks = breaks)]
    }
...
ns.counts2 <- function(mat, col =c('dodgerblue2','white','red'), scale = TRUE ){   
    my.col <- colorRampPalette(col)
    # scale + cluster rows
    scaled <- t(scale(t(mat)))
    dis <- dist(scaled)
    dis <- as.dendrogram(hclust(dis, method = "ward.D"))
    or <- order.dendrogram(dis)
    # reaorder the color map
    if( scale ){
        scaled <- scaled[or,]
        ...
        mat <-  mat[or,]
        mv <- max(abs(as.vector(mat)))
        # log scale breaks
        breaks <- seq.log(1,mv+1,50)
        breaks <- c(rev(-breaks+1)[1:49],breaks-1)
        col.X <- my.col(99)[cut(mat, breaks = breaks)]
    }
    col.X <- matrix(col.X, nrow=nrow(scaled),
                    ncol=ncol(scaled),
...
plot.yax <- function(r.names,r,c, ...){
    ...
    grid.yaxis(at = seq((1/length(r.names))/2,1-(1/length(r.names)/2),
               ...
               label = r.names, main= FALSE, gp = gpar(cex = 0.6))
    upViewport()
}
...
grid.heat2 <- function(XL1, XL2, r.names1=NULL, r.names2=NULL, h.names=NULL ){
    ...
    for(i in seq(XL2)){
        ...
        plot.text("b", r = 4, c = 1, y = 0.8, x = 2, gp = gpar(cex = 1.9))
